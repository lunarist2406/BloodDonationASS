...
interface FormHealthProps {
  ...
  setFormData: React.Dispatch<React.SetStateAction<StatusHealth>>;
}

export default function FormHealth({ formData, setFormData }: FormHealthProps) {
  console.log("📌 Current formData:", formData); // Theo dõi state
        ...
        >
          <DatePicker
            ...
            onChange={(date) =>
              setFormData((prev) => ({
                ...prev,
                lastDonationDate: date?.format("YYYY-MM-DD") || "",
              }))
            }
          />
        </Form.Item>
